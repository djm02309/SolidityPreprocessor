...
public class SolidityPrintVisitor extends SolidityBaseVisitor<String> {
	int wantNonFallback = 0;
	int isFallback = 0;
	int isFirstContract = 0;
	@Override
	public String visitSourceUnit(SolidityParser.SourceUnitContext ctx) {
		...
		if(forCheck.contains("nonfallback")) {
			wantNonFallback = 1;
		}
		...
		if(wantNonFallback == 1) {
			System.out.println("contract NonFallbackEnabled {\n "
				+ "using ForFallBack for ForFallBack.FallBackflag;\r\n"  
				...
				"return self.fb;\r\n" + 
				"}\r\n" + 
				"}\r\n");  //새로 넣는 코드 라이브러리랑 nonfallbackenable 코드로 바꿔야함
		}
		return "";
	}
	...
	public String visitImportDeclaration(SolidityParser.ImportDeclarationContext ctx) {
		if(ctx.getChildCount() == 1) {
			...
			String identi1 = visit(ctx.identifier(0));
			String identi2 = visit(ctx.identifier(1));
			return identi1 + " as " +identi2;
		}
	}
	...
	public String visitContractDefinition(SolidityParser.ContractDefinitionContext ctx) {
		if(wantNonFallback == 1) {
			if(!ctx.getChild(2).toString().equals("is")) {
				return ctx.getChild(0).getText() +" "+ visit(ctx.identifier())+" is NonFallbackEnabled {\n" +
				...
				return ctx.getChild(0).getText()+" " + visit(ctx.identifier())+" is "+
						ctx.inheritanceSpecifier().stream()
						  .map(t ->  ", " + visit(t) )		 
						  .skip(1)							  
						  .reduce(visit(ctx.inheritanceSpecifier(0)), (acc, inheritanceSpecifier) -> acc + inheritanceSpecifier)+", NonFallbackEnabled{\n" +
						  ctx.contractPart().stream()
						  .map(t -> visit(t)+"\n" )
						  .skip(1)	
						  .reduce(visit(ctx.contractPart(0)), (acc, contractPart) -> acc + contractPart)+"function getFB() returns (bool) {\r\n" + 
						  		"return fallBackFlag.getFB();\r\n" + 
						  		"}\r\n" + 
						  		"function setFB() returns (bool) {\r\n" + 
						  		"return fallBackFlag.setFB();\r\n" + 
	...
	@Override 
	public String visitNonFallbackStatement(SolidityParser.NonFallbackStatementContext ctx) { 
		wantNonFallback = 1;
		return "nonfallback " + visit(ctx.statement()) + "if(getFB() == true){\n revert(\"error\");\n }\n "
				+ "function () payable {\r\n" + 
				"setFB();\r\n" + 
				"}"; 
	}
...
//					  .skip(1)
	...
	public String visitBlock(SolidityParser.BlockContext ctx) {
		String result = "";
		if(ctx.getParent() instanceof SolidityParser.FunctionDefinitionContext){
			if(wantNonFallback == 1 && isFallback ==1) {
				result = "setFB(flag); \n";
			}
			isFallback = 0;
		}
		return "{\n"+result+ctx.statement().stream()
				  .map(t ->  visit(t) )		 							  
	...
	public String visitExpression(SolidityParser.ExpressionContext ctx) {
		if(ctx.expression().size() == 2) {
			...
			if(ctx.getChild(0) instanceof SolidityParser.ExpressionContext) { //expression ++|-- expression (fucn) 
				if(ctx.getChild(1).toString().equals("(")) { //expression( functionCallArguments)
					String result = "";
					result = visit(ctx.functionCallArguments());
					if(result.contains("NONFALLBACK")) {
						return visit(ctx.expression(0)) + "( "+ result+" ); \n"
								+ "if(ffb.getData() == 1){\n throw; \n}";
					}
				...
				else if(ctx.getChild(0).toString().equals("nonfallback")) {
					return "nonfallback "+visit(ctx.expression(0));
				}
				else {//++expression 등
